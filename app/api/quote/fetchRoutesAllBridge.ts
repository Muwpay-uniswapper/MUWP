  ...
  for (const chainSymbol in chainDetailsMap) {
    ...
    chainIdToSymbol[
      chainDetails.chainId
        ? Number.parseInt(chainDetails.chainId?.split("0x")[1] ?? "", 16)
        : Number(chainDetails.allbridgeChainId)
    ] = chainSymbol;
  }
  ...
  for (const req of queries) {
    ...
    if (!canBridgeDirectly) {
      ...
      const intermediateSourceToken = ensure(
        sourceChain.tokens.find((t) => t.symbol === intermediateTokenSymbol),
      );
      ...
      req.fromTokenAddress = sourceToken?.tokenAddress ?? req.fromTokenAddress;
    }

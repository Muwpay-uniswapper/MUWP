...
export class StellarAssetService {
	...
	protected readonly soroban?: rpc.Server;
	...
	constructor(options: StellarAssetServiceOptions = {}) {
		...
		this.soroban = options.sorobanUrl
			? new rpc.Server(options.sorobanUrl, {
	...
	async resolveBridgePair(params: {
		...
		const destinationChain =
			details[toSymbol] ??
			details[ChainSymbol.SRB] ??
			details[ChainSymbol.STLR];
		...
		const destinationToken = this.locateDestinationToken(
			destinationChain,
			sourceToken.symbol,
		);
		if (!destinationToken) {
	...
	async estimateBridgeAmount(params: {
		...
		const expectedAmount = parseUnits(
			expected,
			metadata.destinationToken.decimals,
		);
		return {
			expectedAmount,
	...
	async fetchSorobanBalance(
		contractId: string,
		account: string,
	): Promise<SorobanBalance> {
		if (!this.soroban) {
		...
		const entry = (await this.soroban.getContractData(
			contractId,
			key,
		)) as unknown as {
			val?: xdr.ScVal;
	...
	private async buildChainIdLookup() {
		...
		for (const [symbol, chain] of Object.entries(details)) {
	...
	private locateDestinationToken(
		chain: ChainDetailsWithTokens,
		symbol?: string,
	) {
		if (symbol) {

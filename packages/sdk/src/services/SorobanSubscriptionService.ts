...
export class SorobanSubscriptionService {
  ...
  async triggerPayment(params: TriggerPaymentParams): Promise<string> {
    ...
    const tx = new TransactionBuilder(account, {
      ...
      .addOperation(
        contract.call("trigger", nativeToScVal(params.subscriptionId, { type: "u64" })),
      )
      .setTimeout(30)
      .build();
  ...
  async getSubscription(contractId: string, subscriptionId: number): Promise<Subscription> {
  ...
  private async waitForConfirmation(hash: string, maxAttempts = 20): Promise<void> {

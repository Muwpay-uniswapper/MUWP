...
export async function AptosBridgeTxData(step: Step): Promise<Step> {
	...
	const aptosGas = await fetch(
		"https://mainnet.aptoslabs.com/v1/estimate_gas_price",
	).then((res) => res.json());
	...
	if (step.action.fromToken.address === zeroAddress) {
		...
		const txRequest: z.infer<typeof transactionRequestSchema> = {
			...
			gasPrice: toHex(
				BigInt(
					step.estimate?.gasCosts?.[0].price ??
						simulation.request.gasPrice?.toString() ??
						"0",
				),
			),
			to: contract.address,
	...
	const gasPrice = BigInt(
		step.estimate?.gasCosts?.[0]
			.price /* ?? simulation.request.gasPrice?.toString() */ ?? "0",
	);
	const gasLimit =
		BigInt(
			step.estimate?.gasCosts?.[0]
				.amount /* ?? simulation.request.gas?.toString() */ ?? "0",
		) / gasPrice;

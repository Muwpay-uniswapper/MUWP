...
export async function handleAptosRoutes(input: InputType, tempAccount: string) {
	...
	for (const req of queries) {
		...
		if (!possibilities)
			throw new Error(`No routes available for ${req.fromChainId}`);
		let target = possibilities?.[req.fromTokenAddress.toLowerCase()];

		if (!target || target !== req.toTokenAddress) {
			const outputToken = Object.entries(
				AvailablePairs[req.fromChainId] ?? {},
			).find(([_, value]) => value == req.toTokenAddress);
			if (!outputToken)
				throw new Error(
					`No route found for ${req.fromTokenAddress} -> ${req.toTokenAddress}`,
				);
		...
		const _routes = previousRoutes.map(async (prevRoute) => {
			...
			const route: Route = {
				...
				toAmountMin:
					steps[steps.length - 1].estimate?.toAmountMin ?? "0",
				toAmountUSD: formatUnits(
					parseUnits(
						...
						BigInt(
							steps[steps.length - 1].estimate?.toAmount ?? "0",
						),

...
export const columns: ColumnDef<Exchange>[] = [
        ...
        onCheckedChange={(value) => {
          useSwapStore.setState((state) => {
            ...
            if (allowDenyExchanges.deny?.includes(row.original.key ?? "")) {
              allowDenyExchanges.deny = allowDenyExchanges.deny.filter(
                (b) => b !== row.original.key,
              );
            } else {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex flex-row gap-1 max-w-xl overflow-x-scroll">
        {row.original.supportedChains?.map((chain) => (
          <Tooltip key={chain}>
            <TooltipTrigger>
              <ChainIcon id={chain} key={chain} />
            </TooltipTrigger>
            <TooltipContent>Chain ID: {chain}</TooltipContent>
          </Tooltip>
        ))}
      </div>
    ),
